...
variable "task_sizes" {
  type    = "map"
  default = {
    x-small = jsonencode({ cpu = 256, memory = 512 })
    small   = jsonencode({ cpu = 512, memory = 1024 })
  }
}
...
resource "aws_cloudwatch_metric_alarm" "ecs_queue_processing_ecs_ec2_service_task_count_target_queue_messages_visible_lower_alarm" {
  ...
  alarm_actions = [aws_appautoscaling_policy.ecs_queue_processing_ecs_ec2_service_task_count_target_queue_messages_visible_lower_policy.arn]
...
resource "aws_cloudwatch_metric_alarm" "ecs_queue_processing_ecs_ec2_service_task_count_target_queue_messages_visible_upper_alarm" {
  ...
  evaluation_periods = 1
  alarm_actions = [aws_appautoscaling_policy.ecs_queue_processing_ecs_ec2_service_task_count_target_queue_messages_visible_upper_policy.arn]
